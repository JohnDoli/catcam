...
#preload all images
hit_images = {
    1: Image.open("D:\DATA-Honza\Programovani\Python\catcam\osu.cat.code-origo\cat/KeyTapHand1.png"),
    2: Image.open("D:\DATA-Honza\Programovani\Python\catcam\osu.cat.code-origo\cat/KeyTapHand2.png")
}

cursor_images = {}
for key in frame_points.keys():
    cursor_images[key] = Image.open("D:\DATA-Honza\Programovani\Python\catcam\osu.cat.code-origo\cat/{0}/{1}/Hand {2}.png".format(cursor_device, background, key))

default_img = ImageTk.PhotoImage(cursor_images['A'])
...
def iterate():
    ...
    def forceUpdate():
        base_img = cursor_images[f].resize((root.winfo_width(), root.winfo_height()), Image.Resampling.LANCZOS)
        n_base_img = ImageTk.PhotoImage(base_img)
    ...
    if k1_p or k2_p:
        ...
        composite_image = Image.alpha_composite(base_img, final_hit_img)
        composite_image.thumbnail((root.winfo_width(), root.winfo_height()), Image.Resampling.LANCZOS)
        n_base_img = ImageTk.PhotoImage(composite_image)
    else:
        base_img = base_img.resize((root.winfo_width(), root.winfo_height()), Image.Resampling.LANCZOS)
        n_base_img = ImageTk.PhotoImage(base_img)
